...
registerMooseObject("OwlApp", PoroElasticity);

InputParameters
PoroElasticity::validParams()
{
...
PoroElasticity::PoroElasticity(const InputParameters & parameters) : 
Kernel(parameters),
...
Real PoroElasticity::computeQpResidual(){
RealTensorValue I;
RealTensorValue V;
RealTensorValue U;
RealTensorValue F;
...
for(int i=0;i<3;++i){
	U(0,i) = _grad_disp_x[_qp](i);
	U(1,i) = _grad_disp_y[_qp](i);
	U(2,i) = _grad_disp_z[_qp](i);
	for (int j=0;j<3;++j){
		V(i,j) = 0;
		I(i,j) = (i==j);
	}
...
V(_component,0) = _grad_test[_i][_qp](0); V(_component,1) = _grad_test[_i][_qp](1); V(_component,2) = _grad_test[_i][_qp](2);
  ...
  return (_stress[_qp]-F.det()*_p[_qp]*(F.inverse()).transpose()).contract(V);
}
...
Real PoroElasticity::computeQpJacobian(){
RealTensorValue I;
RealTensorValue H;
RealTensorValue Jac;
RealTensorValue V;
...
V(_component,0) = _grad_test[_i][_qp](0); V(_component,1) = _grad_test[_i][_qp](1); V(_component,2) = _grad_test[_i][_qp](2);
  ...
  return (    Jac   -   F.det()*_p[_qp]*(FinvT.contract(H))*FinvT + F.det()*_p[_qp]*(Finv*H*Finv).transpose()     ).contract(V);
}




Real PoroElasticity::computeQpOffDiagJacobian(unsigned int jvar) 
{
RealTensorValue I;
RealTensorValue H;
RealTensorValue Jac;
...
if(jvar == _id_p){
	return -F.det()*_phi[_j][_qp]*FinvT.contract(V);
}
else if(jvar==_id_x){
	component_H = 0;
}
