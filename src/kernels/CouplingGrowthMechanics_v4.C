...
registerMooseObject("OwlApp", CouplingGrowthMechanics_v4);

InputParameters
CouplingGrowthMechanics_v4::validParams()
{
...
CouplingGrowthMechanics_v4::CouplingGrowthMechanics_v4(const InputParameters & parameters) : 
Kernel(parameters),
...
Real CouplingGrowthMechanics_v4::computeQpResidual(){
RealVectorValue disp;
disp(0) = _disp_x[_qp]; disp(1) = _disp_y[_qp]; disp(2) = _disp_z[_qp];

  return 1/_Jk_old[_qp]*(_Psi[_qp])*_test[_i][_qp] + 1/3*(_P[_qp]*(_grad_test[_i][_qp]/_Jk_old[_qp] - _test[_i][_qp]/(std::pow(_Jk_old[_qp],2))*_grad_Jk_old[_qp]  ) )*disp; 
}




Real CouplingGrowthMechanics_v4::computeQpJacobian(){
  return 0.0;
}
...
Real CouplingGrowthMechanics_v4::computeQpOffDiagJacobian(unsigned int jvar){
RealTensorValue H;
RealTensorValue Jac;
int component_H;
