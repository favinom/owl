...
registerMooseObject("OwlApp", CouplingGrowthMechanics_v2);

InputParameters
CouplingGrowthMechanics_v2::validParams()
{
...
CouplingGrowthMechanics_v2::CouplingGrowthMechanics_v2(const InputParameters & parameters) : 
Kernel(parameters),
...
Real CouplingGrowthMechanics_v2::computeQpResidual(){
RealVectorValue disp;
disp(0) = _disp_x[_qp]; disp(1) = _disp_y[_qp]; disp(2) = _disp_z[_qp];

  return 1/_Jk[_qp]*(_Psi[_qp])*_test[_i][_qp] + 1/3*(_P[_qp]*(_grad_test[_i][_qp]/_Jk[_qp] - _test[_i][_qp]/(std::pow(_Jk[_qp],2))*_grad_Jk[_qp]  ) )*disp; 
}




Real CouplingGrowthMechanics_v2::computeQpJacobian(){
  return 0.0;
}
...
Real CouplingGrowthMechanics_v2::computeQpOffDiagJacobian(unsigned int jvar){
RealTensorValue H;
RealTensorValue Jac;
int component_H;
...
Jac = _elastoplasticMaterial[_qp][0].evaluateJac(H,_qp);
  ...
  return (2.0/3.0*_P[_qp].contract(H)-Jac.contract(_F[_qp])/3.0)*_test[_i][_qp]/_Jk[_qp];
}
